...
class Player : GameObject
{
    ...
    public GameState Update(List<GameObject> platforms, List<GameObject> collectibles, List<Enemy> enemies, ref int collectedCount, int totalCollectibles)
    {
        Velocity.Y += Gravity;
        ...
        if (Position.X + Size.X > 1920) Position.X = 1920 - Size.X;
        if (Position.Y < 0) Position.Y = 0;
        if (Position.Y + Size.Y > 1080) Position.Y = 1080 - Size.Y;
